...
pub fun main(authorAddress: Address, readerAddress: Address, postID: UInt64): PostMetadata? {
    ...
    let post = publicRef!.borrowPost(postID: postID, readerAddress: readerAddress) 

    if(post == nil) { return nil }

    let metadata = PostMetadata(_id: post?.id, _title: post?.title, _description: post?.description, 
                                    _author: post?.author, _image: post?.image, _price: post?.price, _data: post?.data, _createDate: post?.createDate,
